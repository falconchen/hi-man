...
class PubHackerNewsTask extends BaseTaskAbstract 
{
    ...
    private function saveToDb($contentHtml) {
        ...
        $postName = 'HackerNews-'.date('YmdHis',$localTimeStamp);
        
        $post = Post::where(['post_name'=>$postName])->first();
        $isCreate = false;
        if($post == null) { 
            $isCreate = true;
            $post = new Post(['post_name'=>$postName]); //新建
            $post->post_author = 12;
        ...
        if ( $isCreate ){
            ...
            $postMeta->meta_value = maybe_serialize( $hackerNewsOSCsync );
            $postMeta->save();
            $this->doSyncPostOsc($postId,$hackerNewsOSCsync);
